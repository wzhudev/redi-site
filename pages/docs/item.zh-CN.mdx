...
类可以直接作为依赖项，注册在注入器中，此时依赖项对应的标识符即为类本身：
...
也可以将 `IdentifierDecorator` 作为标识符，此时，依赖项是一个符合 `ClassDependencyItem` 接口的对象：
...
<Callout>
  请仔细检查被延迟实例化的类的构造函数中是否存在副作用，否则会导致应用不能按你期望的方式运行。
</Callout>

## 值作为依赖项

可以提供一个符合 `ValueDependencyItem` 接口的对象作为值依赖项：
