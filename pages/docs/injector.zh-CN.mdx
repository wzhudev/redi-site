...
注入器在依赖注入系统中起到中心作用，它负责持有绑定并根据标识符来实例化依赖项。

除了之前已经接触到的 `get` API，注入器主要有以下 API：
...
* `createInstance<T extends unknown[], U extends unknown[], C>(ctor: new (...args: [...T, ...U]) => C, ...customArgs: T)`，用注入器来实例化一个类，可以通过追加参数来补充被实例化的类的构造参
