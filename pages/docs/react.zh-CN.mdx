...
组件可以和一组依赖项绑定，这样该组件就提供了一个 RediContext，RediContext 内的注入器含有这组依赖。
...
connectDependencies 和 connectInjector 的区别在于：前者所绑定的组件每次创建时，都会创建一个注入器，各个注入器之间相互独立；后者所绑定的组件并不创建注入器，共享同一个注入器。

另外，当绑定过的组件构成父子关系时，connectDependencies 会自动将两个注入器也构建父子关系。

所以，推荐优先使用 connectDependencies，connectInjector 仅用于根组件。
