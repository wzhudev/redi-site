...
<Callout>在 TypeScript 当中，类型变量和值变量使用不同的命名空间，所以一个接口和一个值有完全相同的名称 `IClipboardInterfaceService` 是合法的，这个小技巧能够将接口和标识符结合起来，降低开发者的心智负担。</Callout>
...
const injector = new Injector([[IClipboardInterface, { useClass: WebClipboardInterfaceService }]])
```
...
const injector = new Injector([[IClipboardInterface, { useClass: MobileClipboardInterfaceService }]])
```
...
<Callout>如果你的应用支持多端，你应该会给不同端的资源设置不同的 bundle。请确保在不同的 bundle 内分别注入对应的实现。</Callout>
