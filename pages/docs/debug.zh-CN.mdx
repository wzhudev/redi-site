...
3. 在 TypeScript 直接存在循环依赖，导致当 `Inject` 装饰器视图在 `XXX` 上记录依赖关系时，`YYY` 还没有被赋值
...
更加推荐的做法是接触文件之间的循环依赖。推荐使用 [bnpm](https://www.npmjs.com/package/dpdm) 来查找文件直接的依赖关系。你可以从  `XXX` 所在文件开始遍历依赖关系，然后把包含这个文件的循环依赖打破。
